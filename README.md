...
**Fields are the unit of everything.** A field is a row with a stable id, and every other table references it by that id, never by name, so renaming a field is a metadata change that leaves history intact. A card's values are dense: one row per active field of the deck, with an empty string for a field the user left blank, so "empty" and "never written" are never confused. Deleting a field deletes everything it influenced: its values, its review history, the configurations and in-progress practices that name it, and the deck's mastery is recomputed from the reviews that remain, so a deleted field leaves no trace in what its neighbors score. ([ADR 009](docs/adr/009-use-field-def-as-sole-source-of-truth-for-fields.md), [ADR 049](docs/adr/049-field-deletion-drops-all-influence.md))

**The practice-time choice is a saved object, and a running practice can't be changed from under you.** Deck configurations are named, reusable templates. A practice copies each configuration's field assignments at the moment it starts, and every card it generates for the rest of its life, retries included, reads that frozen copy. Editing or deleting the configuration never touches a practice built from it; a practice in progress is deleted, after a warning, only when a deck or field it needs is deleted. A link back to the source configuration exists for attribution only; nothing reads it to generate cards, and a material edit to the configuration severs it. ([ADR 013](docs/adr/013-snapshot-practice-config-at-session-start.md), [ADR 040](docs/adr/040-attribution-only-config-lineage.md))
...
**Deletion leaves nothing behind.** Persistent things never depend on ephemeral ones: a subject, deck, field, or card that is deleted takes every record about it, review history and past-practice snapshots included, and a practice in progress goes only when it can no longer function. One service plans that closure and executes it, and the confirmation shows the same plan as counts before anything is removed, so the warning and the deletion cannot disagree. ([ADR 047](docs/adr/047-persistent-entities-never-depend-on-ephemeral-ones.md), [ADR 048](docs/adr/048-review-history-and-snapshots-cascade-with-their-owner.md), [ADR 051](docs/adr/051-one-deletion-closure-plans-and-executes-every-deletion.md))
...
**Continuous integration.** GitHub Actions runs the backend tests against a real PostgreSQL, applies the full migration chain from an empty database, and runs the frontend lint, tests, and build on every pull request and every push to `main` ([ADR 041](docs/adr/041-use-github-actions-for-ci.md)).
