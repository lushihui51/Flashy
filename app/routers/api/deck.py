...
def create_deck(db: SessionDep, current_user: CurrentUserDep, deck: DeckCreate):
    subject = db_read_subject(db, deck.subject_id, current_user.id)
    if not subject:
...
def read_decks(
    ...
    return db_read_decks(db, current_user.id, subject_id)
...
def update_deck(
    db: SessionDep, current_user: CurrentUserDep, deck_id: uuid.UUID, payload: DeckUpdate
):
    deck = db_read_deck(db, deck_id, current_user.id)
    if not deck:
        raise HTTPException(status_code=404, detail="Deck not found")
    if payload.subject_id:
        subject = db_read_subject(db, payload.subject_id, current_user.id)
        if not subject:
            raise HTTPException(status_code=404, detail="Subject not found")
    try:
