...
def create_practice_session(db: SessionDep, payload: PracticeSessionCreate):
    ...
    try:
        return start_practice_session(
            db, strategy, payload.user_id, payload.deck_practice_config_ids
        )
...
def read_practice_session(db: SessionDep, practice_session_id: uuid.UUID):
    session = db_read_practice_session(db, practice_session_id)
    if not session:
...
def read_current_practice_card(db: SessionDep, practice_session_id: uuid.UUID):
    if not db_read_practice_session(db, practice_session_id):
        raise HTTPException(status_code=404, detail="Practice session not found")
    card = db_read_current_practice_card(db, practice_session_id)
    if not card:
...
def rate_practice_card(db: SessionDep, practice_card_id: uuid.UUID, payload: RatingSubmission):
    strategy = get_mastery_strategy()
    try:
        rated, requeued = submit_rating(db, strategy, practice_card_id, payload.ratings)
    except LookupError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
