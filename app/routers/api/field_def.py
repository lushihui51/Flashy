...
def create_field_def(db: SessionDep, deck_id: uuid.UUID, payload: FieldDefCreate):
    if not db_read_deck(db, deck_id):
...
def read_field_defs(db: SessionDep, deck_id: uuid.UUID, include_archived: bool = False):
    if not db_read_deck(db, deck_id):
        raise HTTPException(status_code=404, detail="Deck not found")
    return db_read_field_defs(db, deck_id, include_archived)
...
def reorder_field_defs(db: SessionDep, deck_id: uuid.UUID, ordered_ids: list[uuid.UUID]):
    if not db_read_deck(db, deck_id):
        raise HTTPException(status_code=404, detail="Deck not found")
    field_defs = db_read_field_defs(db, deck_id, include_archived=True)
...
def read_field_def(db: SessionDep, field_id: uuid.UUID):
    field_def = db_read_field_def(db, field_id)
    if not field_def:
...
def update_field_def(db: SessionDep, field_id: uuid.UUID, payload: FieldDefUpdate):
    field_def = db_read_field_def(db, field_id)
    if not field_def:
        raise HTTPException(status_code=404, detail="Field not found")
...
def archive_field_def(db: SessionDep, field_id: uuid.UUID):
    field_def = db_read_field_def(db, field_id)
    if not field_def:
        raise HTTPException(status_code=404, detail="Field not found")
...
def hard_delete_field_def(db: SessionDep, field_id: uuid.UUID):
    field_def = db_read_field_def(db, field_id)
    if not field_def:
        raise HTTPException(status_code=404, detail="Field not found")
