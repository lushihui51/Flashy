...
from app.database_ops.practice_run import (
    ...
    db_read_practice_run_with_decks,
    db_read_practice_runs_with_decks,
)
...
@router.post("/practice_runs", response_model=PracticeRunRead, status_code=201)
def create_practice_run(
    db: SessionDep, current_user: CurrentUserDep, payload: PracticeRunCreate
):
...
@router.get("/practice_runs", response_model=list[PracticeRunSummary], status_code=200)
def read_practice_runs(
    db: SessionDep,
...
@router.get(
    "/practice_runs/{practice_run_id}",
    response_model=PracticeRunSummary,
    status_code=200,
)
def read_practice_run(
...
@router.delete("/practice_runs/{practice_run_id}", status_code=204)
def delete_practice_run(
    db: SessionDep, current_user: CurrentUserDep, practice_run_id: uuid.UUID
):
...
@router.get(
    "/practice_runs/{practice_run_id}/state",
...
@router.get(
    "/practice_runs/{practice_run_id}/breakdown",
    response_model=PracticeRunBreakdown,
    status_code=200,
)
...
@router.post(
    "/practice_runs/{practice_run_id}/rerun",
