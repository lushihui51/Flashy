...
Work on branch `feat/practice-setup`. The follow-up run-flow task will branch as `feat/practice-run` off this one once merged. Commit at each phase boundary with the message given in the phase.
...
6. **The four field arrays of a config are pairwise disjoint** — the builder enforces this _by construction_: assigning a field to a section moves it (a field lives in exactly one place: Unassigned or one section), never copies it.
...
Build it as one surface used for both create and edit (edit loads an existing config and pre-populates).
...
**Acceptance:** create, edit, and delete round-trip against the real backend; a seeded deck with an archived field never shows it in the builder; frequency pruning verified when moving fields out of a pool; each backend validation rule has a matching disabled-save state client-side; duplicate-name error renders inline; MSW tests for the payload mapping (board state → arrays); every assignment reachable by tap/click alone on a phone-width viewport.
