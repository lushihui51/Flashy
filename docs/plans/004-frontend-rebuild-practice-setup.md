...
6. **The four field arrays of a configuration are pairwise disjoint** — the builder enforces this _by construction_: assigning a field to a section moves it (a field lives in exactly one place: Unassigned or one section), never copies it.
...
**The assignment board** _(revised 2026-08-25 — the original drag-table spec shipped and failed: HTML5 DnD is mouse-only, the embedded select swallowed the press so the chip was effectively undraggable, and the table layout doesn't fit narrow viewports; drag is replaced by tap-to-assign):_

- **Five stacked sections, one layout at every viewport** — Unassigned · `prompt_fields` · `answer_fields` · `prompt_pool` · `answer_pool`. No table, no columns, no responsive fork.
- Each live field_def is a chip (by `name`). **The whole chip is a single tap/click target** that opens a destination picker (bottom sheet on narrow viewports, popover on wide) listing the other four sections; choosing one moves the chip. No control is embedded inside a chip. Move semantics per invariant 6.
- **Frequency:** a pool section renders its checkbox list (labeled 1…n, n = fields currently in that section) in its footer **only when it has ≥1 field**. Empty sections show a short hint line — never `N/A`. When a field leaves a pool and n shrinks, **prune any checked value now greater than n**. Checked values become the `*_pool_counts` array (sorted ascending). The one-line explanation of the random draw sits beside the first visible frequency row.
- **Native HTML5 drag-and-drop is removed entirely** — no fallback hybrid. Pointer-based drag (dnd-kit) between sections may return later as a wide-viewport enhancement layered on the same assignment state; it is not part of this task and not in any acceptance criteria.
- **Payload mapping:** section contents → `prompt_field_ids`, `answer_field_ids`, `prompt_pool_ids` + `prompt_pool_counts`, `answer_pool_ids` + `answer_pool_counts`. Uuids only.
...
- **A configuration is not a top-level resource.** The create sheet behind the top bar's `+` lists **Subject, Deck, Card, Practice** — in the order they nest — and deliberately no configuration row: one belongs to a single deck and only means anything beside that deck's fields. It has exactly two doors, the deck's own `Configurations` tab and (from Phase 3) New practice. Do not add a third.

**Acceptance:** create, edit, and delete round-trip against the real backend; a configuration referencing a since-archived field never shows it in the builder; frequency pruning verified when moving fields out of a pool; each backend validation rule has a matching disabled-save state client-side; duplicate-name error renders inline; MSW tests for the payload mapping (board state → arrays); every assignment reachable by tap/click alone on a phone-width viewport.
