...
- **D12's `practice_deck` line is wrong as written** (sync 2026-09-18): `practice_deck.deck_id` is nullable `ON DELETE SET NULL`, not `CASCADE` from `deck` — the snapshot outlives its deck as history (ADR 015, `app/models/practice_deck.py`). What cascades on `practice_deck` is `practice_run_id`, with its run (ADR 015 amended). The rest of D12 stands.
- **`card_field_mastery` is `mastery_log`** (ADR 042, task 010 T1; sync 2026-09-18) wherever D12, D13, and §2.2 name it: an append-only ledger whose `card_id`/`field_def_id` still cascade, so D12's cascade reasoning is unchanged. D13's "last practiced = `MAX(card_field_mastery.updated_at)`" would now be the `reviewed_at` of the pair's max-`id` `mastery_log` row — still derived, still not stored, still unbuilt.
- **§4.7's "there is no `Untitled card` anywhere in the app" no longer holds** (sync 2026-09-19): task 006 T8 reintroduced it as the completion breakdown's row fallback for a card whose primary field is blank (`frontend/src/components/practice/RunBreakdown.tsx`). The deck detail table still renders `empty` per Phase 2.5; the two surfaces deliberately differ.
- **D12's SET NULL half is gone** (ADR 047, ADR 048, task 013 T4; sync 2026-09-21): `practice_deck.deck_id` and `review_log.card_id`/`field_def_id` are `NOT NULL ON DELETE CASCADE`, a card or deck delete removes its review rows and snapshots, and a run left with no decks is deleted — so the "`review_log` — never deleted" line, the card `DELETE` line's "review_log rows remain with `card_id` null", the D12 constraint list, and the 2026-09-18 bullet above about `practice_deck.deck_id` being nullable are all superseded.
- **Field removal is a hard delete, and the editor's save confirm counts the closure** (ADR 049, task 013 MD-2, T5, T7; sync 2026-09-21): D11 and the "`field_def.archived_at` soft-archive is unchanged" line describe code that stays but is exposed nowhere — the editor's remove-field goes through the batch edit into the deletion closure, which deletes the field's values, review rows, and ledger rows, the configurations and active runs naming it, and rebuilds the deck's mastery. The §4.7 confirm "naming the count of affected cards" and Phase 7.5 §3's "This deletes 1 field and affects 2 cards" are replaced by the sentence in task 013's Frontend copy contract, which also names the configurations deleted and active practices deleted.
- **Later cycles moved what §4 and Phase 2 describe** (sync 2026-09-23): return addresses ride the URL through `internalReturnTo`, not router state (§4.4, Phase 5.5 §4; ADR 024, task 004 T4); the library's Decks tab renders plain links, not the 2.7 §3 deck rows (§4.8); detail-page action rows hold Practice and Edit only, with adds inside the collection as **Add deck**, **Add card**, and **New configuration** (Phase 2.5 §1.3, 2.6 §2.4/§4, §4.8; ADR 023); `CreateSheet` has a fourth Practice row (§4.1; task 004); the subject and deck delete confirms fetch `readDeletionImpact` and render `deletionSummaryText` (§4.2, Phase 7.6, open question 4; task 013 T6); the deck page's breadcrumb is the two-link `Your library › Subject` chain (Phase 2.5 §1.1; task 005 T1); and D12's "deferred" terminal transition for a stranded run is built in `get_current_practice_card` (ADR 015 as amended).
- **D10's list of paths that maintain density gained the standalone route** (ADR 057, task 016 T1; sync 2026-09-24, final pass): `POST /decks/{deck_id}/fields` writes a `""` row for every existing card in the same transaction through `app/services/field_def_create.py`, so D10 and §2.3's "§2.2 only covers cards created atomically with their deck" now name three paths: deck create, the batch edit's field-add, and the standalone route.

## ADRs

- **ADR 015 — deck deletion cascades owned rows, preserves history via SET NULL.** Records **D12** (deletion policy), implemented in Phase 1.5. Its SET NULL half is superseded by ADR 047 and ADR 048 (task 013 T4; sync 2026-09-21, see the bullet above); the cascade of deck-owned rows stands.
