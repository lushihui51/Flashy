...
## Superseded since (sync 2026-09-15)
...
- **More ADR 050 and ADR 048/049 fallout** (sync 2026-09-23): the model block's indexes are `UNIQUE (card_id, field_def_id, reviewed_at)` (`uq_mastery_log_card_field_reviewed`) and `ix_mastery_log_run (practice_run_id, reviewed_at)`, and its "raw rating history survives in `review_log`" no longer holds — a deleted card's review rows cascade with it (ADR 048); `db_append_mastery_log` also takes `review_group_id`; `record_review_group` takes the card lock and stamps `reviewed_at`; `rebuild_deck_mastery` is a second, deck-scoped rebuild path (ADR 049). T1's Notes (`Identity()` BIGINT id) and T3's Details (`id < bound` probe on `ix_mastery_log_card_field`) read accordingly; T3's Notes overstate the statement-count test, which asserts only that the count is equal across run sizes, not that it is 2. The ADR 042 bullet's "ADR 011 intact" is now "amended by ADR 048 and ADR 049".
- **The write-path names carry the `stage_` prefix** (ADR 055, task 015 T4; sync 2026-09-24): `db_append_mastery_log` is `db_stage_append_mastery_log` and `db_clear_mastery` is `db_stage_clear_mastery` wherever this file names them; signatures are unchanged. Since task 015 T7 (sync 2026-09-24, final pass), the per-card advisory lock the write-path contract describes is `db_lock_card` in `app/database_ops/mastery_log.py`, taken by `apply_rating` and by `record_review_group` before it; `db_log_review_group` there is `db_stage_log_review_group`; and `rebuild_mastery`'s attribution join is `db_read_run_ids_for_practice_cards` in `app/database_ops/practice_card.py`.
...
- Notes: `db_fetch_mastery_before_bound` uses the `DISTINCT ON` joined-to-bounds-relation shape, not `LATERAL` — a plain inner join naturally omits a pair with no row below its bound (no left join / null-row handling needed), and the caller already knows the full domain independently, so absence from the returned dict is enough. Measured (and asserted in the statement-count test) at exactly 2 statements per breakdown request regardless of run shape: R's own rows, then the bounds lookup. The per-field-bound test was mutation-tested — forcing a single run-wide bound instead of each pair's own makes it fail with the wrong number, confirming it's a real regression guard, not vacuous. `app/routers/api/practice_run.py` needed a matching one-line change (`get_practice_run_breakdown` gained a required `strategy` parameter, so its one call site needed `get_mastery_strategy()` threaded through) — mechanically necessary, not a design choice, so not called out as its own decision. The delta-semantics tests (`TestBreakdownMastery`) build `practice_run`/`practice_card`/`mastery_log` rows directly rather than through the generation/rating HTTP flow, for exact control over row ordering and attribution — the thing the contract is actually about — with expected mastery values computed via the same `MasteryStrategy.expand`/`apply_review` calls `apply_rating` itself makes (an independent oracle for the bound/attribution logic under test, without hand-computing EMA arithmetic). The "one card on the two-field `existing_field_defs` deck" phrasing is read as describing that _shape_ (2 active fields, 1 card), not literally reusing the fixture: `existing_field_defs` and `session_cards`/`session_config` both depend on `existing_deck`, so using both fixtures in one test would merge onto a single 10-field deck instead of two separate ones: `test_delta_computation_statement_count_is_fixed` instead builds its own fresh 2-field deck via `POST /api/decks` (`multi_subject_library`'s own pattern for a second deck), and reuses `session_cards`/`session_config` as-is for the 8-field side. `npm run gen:api`, `npx vitest run`, and `npm run lint` are all clean; `npm run build`'s `tsc -b` step is not — `RunBreakdown.test.tsx`'s existing fixtures (T4's own file) don't yet supply the newly-required `mastery`/`delta`/`fields` fields, and T4's Done-when (unlike T3's) explicitly requires a clean build, so it's the task that resolves this. Full `pytest`: 280 passed.

### T4 — Breakdown UI: ranked list (ADR 044, MD-1, MD-2) — after T3

- [x] **Goal:** the breakdown renders as one delta-ranked list with badges, and the detail sheet shows per-field mastery/deltas.
...
- Notes: The sort control isn't specified down to markup, so two judgment calls: it's a `role="radiogroup"`/`role="radio"` button group (not `role="tablist"`, since a persistent sort choice isn't a content-swapping tab, and not a native `<select>`, since no such precedent exists anywhere in this app and the three labels stay visibly scannable as buttons — matching this codebase's own established hand-rolled-button-group style, used identically in four other places); the outcome badge is a plain neutral bordered pill (no per-bucket color-coding), and mastery/delta render in the row's default text color with no added color-by-sign — both to avoid inventing a color semantic the contract doesn't specify, given Done-when only checks label/number text, not color. `PracticeRunPage.test.tsx`/`PracticeDetailsPage.test.tsx` needed both: their `getByRole('tab', ...)` selectors (removed with the tabs) and their `breakdownState()`/`breakdown()` fixtures, which lacked the now-required `mastery`/`delta`/`fields` fields and would otherwise render `NaN`/crash at runtime (MSW doesn't type-check its mocked JSON, so this only surfaces at test run time, not compile time) — the task's Files line anticipated only "selectors," so the fixture-completeness fix is noted here as the same kind of mechanical necessity, not a design choice. Verified against the dev server with a real, already-completed 20-card run already in the dev database (no data seeded or mutated — only GET requests and read-only UI interaction): screenshots of both call sites (`/practice/{id}` and `/practice/{id}/run`), the sort control switching from Gains-first to Mastery and correctly re-ordering, and the detail sheet's new Fields section, all sent to the user; zero browser console errors. `npx vitest run` (418 passed), `npm run lint`, and `npm run build` are all clean. Full backend `pytest` (282 passed) reconfirmed unaffected, since this task touched frontend only.
