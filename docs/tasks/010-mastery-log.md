...
- **The write-path names carry the `stage_` prefix** (ADR 055, task 015 T4; sync 2026-09-24): `db_append_mastery_log` is `db_stage_append_mastery_log` and `db_clear_mastery` is `db_stage_clear_mastery` wherever this file names them; signatures are unchanged. The per-card advisory lock is still an inline `db.execute(text(...))` in `app/services/mastery.py` until task 015 T7 puts it behind `db_lock_card`.
