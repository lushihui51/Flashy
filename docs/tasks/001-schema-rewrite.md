...
- **`card_field_mastery` is gone, replaced by the append-only `mastery_log` ledger** (ADR 042, task 010 T1/T2; sync 2026-09-15): this file's Contracts block — the `card_field_mastery(card_id, field_def_id, ...)` DDL, "driving a query from `card_field_mastery` is a bug," and the plain-upsert write path it describes — is no longer how mastery is stored. `mastery_log` is one row per (card, field) state _change_ (a BIGINT identity id as its ordering key, a nullable `practice_run_id` for run attribution), never updated in place; current state is the latest row per pair. `rebuild_mastery`'s truncate-and-replay shape (T4 here) still holds, now truncating/repopulating `mastery_log` instead. Invariant 2 ("mastery is a disposable, rebuildable cache of `review_log`") is unchanged; invariant 8 (arithmetic confined to `MasteryStrategy`) is unchanged and, per ADR 042, still enforced by a source-scan test (now over `app/database_ops/mastery_log.py`/`app/models/mastery_log.py`, not the old module names).
