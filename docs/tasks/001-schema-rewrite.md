...
- **`SessionStatus` is two values** — `active`, `completed`. `abandoned` was never built; the enum, its CHECK constraint, and everything task 004 onward built assume two values (`app/models/practice_run.py` — `RunStatus` since ADR 038).
...
- **`practice_session` is `practice_run`, and `practice_deck` does have a `source_config_id`** (sync 2026-09-14): the table, its modules, and `SessionStatus`→`RunStatus` were renamed by ADR 038 (task 009 T2), with the `practice_session_id` columns, `uq_practice_card_practice_session_id`, and the `(practice_session_id, status, position)` index renamed to match. The Contracts line "`practice_deck` has **no** `source_config_id`" is no longer true: ADR 040 (task 009 T6/T7) added it as nullable, ON DELETE SET NULL attribution that generation never reads and a material config edit nulls; invariant 5's self-contained-snapshot rule is otherwise intact.
- **`card_field_mastery` is gone, replaced by the append-only `mastery_log` ledger** (ADR 042, task 010 T1/T2; sync 2026-09-15): this file's Contracts block — the `card_field_mastery(card_id, field_def_id, ...)` DDL, "driving a query from `card_field_mastery` is a bug," and the plain-upsert write path it describes — is no longer how mastery is stored. `mastery_log` is one row per (card, field) state _change_ (ordered by `reviewed_at`, stamped strictly increasing per card, with a uuid `id` — ADR 050; a nullable `practice_run_id` for run attribution), never updated in place; current state is the latest row per pair. `rebuild_mastery`'s truncate-and-replay shape (T4 here) still holds, now truncating/repopulating `mastery_log` instead. Invariant 2 ("mastery is a disposable, rebuildable cache of `review_log`") is unchanged; invariant 8 (arithmetic confined to `MasteryStrategy`) is unchanged and, per ADR 042, still enforced by a source-scan test (now over `app/database_ops/mastery_log.py`, `app/models/mastery_log.py`, and `app/services/mastery.py`, not the old module names).
- **`review_log` no longer outlives its card or field, and `practice_card_id` is gone** (ADR 047, ADR 048, task 013 T4; sync 2026-09-21): `card_id` and `field_def_id` are `NOT NULL ON DELETE CASCADE` and the `practice_card_id` column was dropped, so the SET NULL bullet above no longer holds; the "Field hard-delete" bullet is superseded by ADR 049's deletion closure (task 013 T3/T5) — the archive endpoints stay in the code, unexposed.
- **Invariant 6 ("field deletion is archival") no longer holds** (ADR 049, task 013 T5; sync 2026-09-21): field removal is a hard delete through the deletion closure, which also deletes the configurations and active runs naming the field, scrubs it from `review_log.shown_prompt_ids`, and rebuilds the deck's mastery; `archived_at`, its partial unique index, and the archive endpoints stay in the code as an option not taken. The invariant's "only when values, mastery, config references, and review logs are all empty" gate now describes only the dormant `DELETE /fields/{id}/hard`.
...
- **ADR 010 — archive fields instead of hard-deleting** (invariant 6). Superseded by ADR 049 (task 013 T5; sync 2026-09-21): field removal is a hard delete through the deletion closure; the archive code stays, unexposed.
...
- **ADR 008 — sparse positions for practice-card ordering** (T6's position scheme; amended by `9e9401f` to state the exact requeue-position guarantee; its deferred minimum-gap follow-up is decided by ADR 037).
