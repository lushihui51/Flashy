...
- **Carried invariant 2's "no cardless session" is not enforced** (sync 2026-09-23): `start_practice_run` never checks the generated card count, so a deck with no cards yields an empty run that completes on its first read. Deferred to a planning session; see `docs/cc/2026-09-23-final-sync-before-dogfooding.md`.
