...
- **T5's Done-when has three superseded criteria, and T1's Notes name pre-rename helpers** (sync 2026-09-23): the completed body renders `RunBreakdown`, not a summary line (task 006 T8); `SessionDeckChips` takes `decks` only, so no deleted-deck count renders (task 013 T6); the run page is the real surface and no `practice-run` tag exists (task 006). `_summaries_for_sessions`, `db_read_practice_sessions_with_decks`, and `db_read_practice_session_with_decks` are `_summaries_for_runs`, `db_read_practice_runs_with_decks`, and `db_read_practice_run_with_decks` (ADR 038).
- **Carried invariant 2's "no cardless session" is not enforced** (sync 2026-09-23): `start_practice_run` never checks the generated card count, so a deck with no cards yields an empty run that completes on its first read. Deferred to a planning session; see `docs/cc/2026-09-23-final-sync-before-dogfooding.md`. Enforced since task 015 T1 (ADR 056): start and rerun refuse with `no_cards`.
